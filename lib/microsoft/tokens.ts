...
import { CredentialDecryptionError, openRefreshToken, sealRefreshToken, type SealedToken } from "@/lib/google/vault";
import { logAudit } from "@/lib/audit/log";
import { DataSourceUnavailable, aadFor } from "@/lib/google/tokens";
 ...
 * Outlook, say). Narrower than `clearTokenCache` so one org's rejection can't cost every
...
/**
 * Hands a caller a usable Microsoft access token, refreshing when the cached one is close to
...
export async function getAccessToken(
  ...
  if (!row) throw new DataSourceUnavailable("No Microsoft account is connected.", "missing");
  if (row.state !== "active")
    throw new DataSourceUnavailable(`The Microsoft connection is ${row.state}.`, "revoked");
  ...
  try {
    ...
    logFailure("Microsoft credential decrypt", {
      provider: "microsoft",
    ...
    if (updateError) {
      logFailure("Microsoft credential decrypt state update", {
        provider: "microsoft",
    ...
    throw new DataSourceUnavailable(
      "Microsoft needs to be reconnected before this integration can run.",
      "reconnect",
    );
  }
  ...
  if (!refreshed.ok) {
    ...
    // settings screen as needing reconnection. A transient one — Microsoft's token endpoint
  ...
  if (refreshed.refreshToken) {
    const sealed = sealRefreshToken(refreshed.refreshToken,
      aadFor(row.org_id as string, row.provider as string, row.external_account_id as string));
    const { error: rotationError } = await db.from("connected_data_source").update({
      token_sealed: sealed.tokenSealed, dek_sealed: sealed.dekSealed, kek_version: sealed.kekVersion,
      updated_at: new Date(now).toISOString(),
    }).eq("id", row.id).eq("org_id", orgId).eq("provider", "microsoft");
...
async function refreshAccessToken(
  ...
  const response = await doFetch(MICROSOFT_TOKEN_ENDPOINT, {
    method: "POST",
  ...
  if (!response.ok) {
    ...
    // `invalid_grant` is Microsoft's own signal that the refresh token itself is dead — every
    ...
    return { ok: false, error: `Microsoft refused the refresh: ${detail}`, permanent };
  }
  if (typeof body?.access_token !== "string")
    return { ok: false, error: "Microsoft returned no access token.", permanent: false };
...
/**
 * Upserts the org's grant. Scopes are stored as Microsoft actually granted them, never as
 * they were requested.
 */
export async function storeGrant(db: SupabaseClient, args: StoreGrantArgs): Promise<string> {
  const aad = aadFor(args.orgId, "microsoft", args.externalAccountId);
  const sealed: SealedToken = sealRefreshToken(args.refreshToken, aad);

  // One org holds one active Microsoft connection. The upsert below keys on
  // (org_id, provider, external_account_id), so connecting a *different* Microsoft account
  ...
  const { data: others } = await db.from("connected_data_source")
    .select("id").eq("org_id", args.orgId).eq("provider", "microsoft").eq("state", "active")
  ...
  const { data, error } = await db.from("connected_data_source").upsert({
    org_id: args.orgId, provider: "microsoft",
...
/** Decode only tokens received directly from Microsoft's token endpoint over TLS. */
export function decodeIdToken(idToken: unknown): { sub?: string; oid?: string; preferred_username?: string; email?: string } {
