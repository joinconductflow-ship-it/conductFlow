    ...
    <Card style={{ borderLeft: health === "connected" ? "2px solid var(--ok)" : connections.length ? "2px solid var(--warn)" : "2px solid var(--border-strong)" }}>
      ...
      {health === "not_connected" ? (
        ...
        <div style={{ marginTop: "var(--space-4)" }}>
          <p style={{ color: "var(--muted)" }}>Microsoft needs to be reconnected before ConductFlow can read channels.</p>
        ...
        <form onSubmit={(event) => {
          ...
          {!loading && !channels.length && !error && health === "connected" && <p style={{ color: "var(--muted)", marginTop: "var(--space-2)" }}>No channels available. Check your Teams membership, then reload.</p>}
          <div style={{ display: "flex", gap: "var(--space-3)", marginTop: "var(--space-4)", flexWrap: "wrap" }}>
            ...
            <a href="/auth/microsoft/connect" className="cf-btn" style={buttonStyle("ghost")}>Reconnect Microsoft</a>
          </div>
        </form>
      </>}
      ...
      {(error || health === "connection_issue") && <p role="alert" className="cf-empty-state" style={{ color: "var(--danger-text)", marginTop: "var(--space-3)" }}>{error ?? microsoftHealthMessage(health)}</p>}
